...
func _ready() -> void:
	...
	for l in labels:
		l.visible = false
	labels[tutorialglobal.text_current].get_node("Area2D").body_entered.connect(_on_area_triggered)
	
func _on_area_triggered(body):
	var label = labels[tutorialglobal.text_current]
	label.visible = true
	label.get_node("AnimationPlayer").play(anim_names[tutorialglobal.text_current])
	...
	tutorialglobal.text_current += 1
	if tutorialglobal.text_current < labels.size():
		labels[tutorialglobal.text_current].get_node("Area2D").body_entered.connect(_on_area_triggered)
